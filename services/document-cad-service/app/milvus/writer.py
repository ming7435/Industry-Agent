...
class MilvusVectorWriter:
    ...
    def existing_record_ids(self, record_ids: Iterable[str]) -> set[str]:
        """Return the requested primary keys that currently exist."""
        ...
        if get_method is None:
            raise MilvusWriteError("This Milvus client does not support primary-key verification.")
        existing: set[str] = set()
        for batch in _batched(ids, self.config.batch_size):
            rows = get_method(
                collection_name=self.config.collection_name,
                ids=batch,
                output_fields=[self.config.primary_field],
            )
            for row in rows or []:
                value = row.get(self.config.primary_field) if isinstance(row, dict) else None
    ...
    def _create_client(self) -> Any:
        ...
        try:
            ...
            return MilvusClient(**connection_args, db_name=self.config.database)
