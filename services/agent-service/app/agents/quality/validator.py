...
class QualityValidator:
    ...
    def validate(
        ...
        workorder_compliance = str(workorder.get("status") or "") in cls.COMPLETED_STATUSES
        ...
        steps = list(workorder.get("steps") or [])
        if not steps:
            failed.append("workorder_steps_missing")
            findings.append("工单缺少维修步骤记录")

        feedback = str(workorder.get("repair_feedback") or "").strip()
        ...
        if device_status.get("found") is False or device_status.get("success") is False:
            ...
            alarm_cleared = bool(repair_check.get("alarm_cleared"))
            ...
            alarm_cleared = alarm_code in (None, "", "None")
        ...
        sop_compliant = bool(sop_check.get("passed") or repair_check.get("sop_compliant"))
