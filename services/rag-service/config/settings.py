...
class Settings(BaseSettings):
    ...
    """单次 SiliconFlow 向量请求的超时秒数。"""

    embedding_max_attempts: int = 6
    """远程向量请求发生临时网络错误时的最大尝试次数。"""
    ...
    ingestion_cache_dir: str = str(SERVICE_ROOT / "data" / ".ingestion_cache")
    """离线流水线的本地中间缓存目录。"""
    ...
    def milvus_search_collections(self) -> list[str]:
        ...
        if not explicit and data_root.is_dir():
            for folder in sorted(data_root.iterdir(), key=lambda item: item.name.casefold()):
                if not folder.is_dir() or folder.name.lower() in {
                    "index",
                    ".git",
                    ".ingestion_cache",
                    "__pycache__",
