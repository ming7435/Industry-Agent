...
7. 当证据来自多个来源时，必须合并多个来源中的互补步骤，不能只选一条；按“直接相关、间接相关、待确认”分层说明。
8. 没有明确报警码或设备范围时，回答必须写清适用范围，不能把单个历史案例直接当成当前设备事实。
9. 对证据已经给出的检查动作尽量完整保留，不要为了简短省略前置条件、确认标准、异常分支和复测要求。
"""
...
三、排查步骤：按执行顺序编号，每步写明“做什么、如何确认正常/异常、异常时如何处理”；合并多个来源时不得只保留一个步骤。
四、维修建议：包含所需备件/工具、关键参数、复位或试运行要求。
五、安全提示：列出本次作业必须遵守的安全措施。

六、适用范围与证据局限：说明这些步骤适用于什么设备/场景，哪些结论仍需现场确认。
