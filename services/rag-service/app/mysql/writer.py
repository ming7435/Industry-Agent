...
class MySQLRagWriter:
    ...
    def find_complete_document(
        ...
        *,
        ...
        """Return completion stats when content and pipeline configuration match."""
        ...
        try:
            ...
            with connection.cursor() as cursor:
                cursor.execute(
                    """
                    SELECT chunk_count, vector_count, metadata_json FROM rag_documents
            ...
            if not row:
                return None
            metadata = _decode_json_mapping(row[2])
            if pipeline_signature is not None and metadata.get("pipeline_signature") != pipeline_signature:
                return None
            with connection.cursor() as cursor:
                cursor.execute(
                ...
                chunk_ids = [str(item[0]) for item in cursor.fetchall()]
            return {
...
def _decode_json_mapping(value: Any) -> dict[str, Any]:
    """Decode a JSON column returned as text, bytes, or an existing mapping."""
