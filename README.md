...
- Router、Knowledge、CAD、Maintenance、WorkOrder、Quality、Report、Experience Agent
...
- 短期 Memory、长期 Memory、Experience Agent 维修经验提取并回写 RAG
...
用户提问入口会经过 `Router -> Orchestrator -> 目标Agent`；异常事件入口会经过 `Diagnosis -> Knowledge -> CAD -> Maintenance -> WorkOrder -> Quality -> Report -> Experience -> Memory/RAG`。
